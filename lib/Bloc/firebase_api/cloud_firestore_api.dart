...
import 'package:foodgpvjreviews/FoodPlace/model/food_place.dart';
...
class CloudFirestoreApi {
  ...
  static const String foodPlaces =
      "foodPlaces"; //nombre de la coleccion de lugares (Places) en Firebase

  final FirebaseFirestore _dataBase = FirebaseFirestore.instance;
  final FirebaseAuth _auth = FirebaseAuth.instance;

  //primer metodo Actualizar datos de usuario
  void updateUserData(model_user.User user) async {
    ...
    referenceUsers
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        return await reference.set({
          ...
          'email': user.email,
          'photoUrl': user.photoURL,
          //'myFoodPlaces': user.myPlaces,
          //'myFavoriteFoodPlaces': user.myFavoritePlaces,
          'lastSignIn': DateTime.now()
        }, SetOptions(merge: true));
      }
    });
  ...
  Future<void> updateUserFoodPlaceData(FoodPlace place) async {
    CollectionReference referenceFoodPlaces = _dataBase.collection(foodPlaces);

    User? currentUser = _auth.currentUser;

    if (currentUser != null) {
      await referenceFoodPlaces.add({
        'name': place.name,
        ...
        _documentReference.get().then((_documentSnapshot) {
          ...
          refUsers.update({
            ...
            'myFoodPlaces': FieldValue.arrayUnion(
                [_dataBase.doc("$foodPlaces/${_documentSnapshot.id}")])
          });
        });
      });
    } else {
      log("No hay usuario logueado",
          name: "Metodo updateUserFoodPlaceData: cloud_firestore_api.dart");
    }
  }
